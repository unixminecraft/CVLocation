...
public final class BukkitLocationPlugin extends JavaPlugin implements IPCInterface {
    ...
    public void onEnable() {
        ...
        this.ipcPlugin.registerInterface(IPCConstants.REQUEST_CHANNEL, this);
    }
    ...
    public void onDisable() {
        this.ipcPlugin.deregisterInterface(IPCConstants.REQUEST_CHANNEL);
    }
    ...
    public void process(final String channel, final String message) {
        ...
        if (!channel.equalsIgnoreCase(IPCConstants.REQUEST_CHANNEL)) {
            logger.log(Level.WARNING, "Invalid channel for Bukkit Location plugin: " + channel + ". Channel should be " + IPCConstants.REQUEST_CHANNEL);
            return;
        }
        ...
        builder.append(IPCConstants.RESPONSE_CHANNEL).append(IPCConstants.SEPARATOR);
        builder.append(rawSenderId).append(IPCConstants.SEPARATOR);
        builder.append(rawTargetId).append(IPCConstants.SEPARATOR);
        builder.append(worldName).append(IPCConstants.SEPARATOR);
        builder.append(x).append(IPCConstants.SEPARATOR);
        builder.append(y).append(IPCConstants.SEPARATOR);
        builder.append(z).append(IPCConstants.SEPARATOR);
        builder.append(yaw);
        ...
        if (regions.size() == 0) {
            logger.log(Level.WARNING, "Sender IUUD: " + rawSenderId + " / Target UUID: " + rawTargetId + " / World: " + worldName + " / No regions present.");
            builder.append(IPCConstants.REGIONS_UNKNOWN);
            this.ipcPlugin.sendMessage(builder.toString());
            return;
        }
